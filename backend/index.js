...
app.post('/salvar-velocidade', async (req, res) => {
  const { user_id, velocidade, duracao, maior_velocidade, cont_velocidade, data } = req.body;

  // Verificação de dados
  if (!user_id || !duracao || !data) {
  ...
  // Se a cont_velocidade não for fornecida, define como 0
  ...
  try {
    ...
    const query = `
      INSERT INTO corridas (user_id, velocidade, duracao, maior_velocidade, cont_velocidade, data)
      VALUES (?, ?, ?, ?, ?, ?)
    `;

    // Executa a query com a velocidade corrigida
    const [results] = await connection.execute(query, [user_id, velocidadeFinal, duracao, maior_velocidade, contVelocidadeFinal, data]);
