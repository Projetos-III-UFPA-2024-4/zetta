...
const CadastroMotorista = ({ navigation }) => {
    ...
    const gerarId = async (tipo) => {
        try {
            const response = await fetch(`http://192.168.1.227:5000/ultimo-id/${tipo}`);
    ...
    const verificarIdExistente = async (novoId) => {
        try {
            const response = await fetch(`http://192.168.1.227:5000/verificar-id/${novoId}`);
            const data = await response.json();
            return data.exists;
        } catch (error) {
    ...
    const salvarNoBanco = async () => {
        ...
        try {
            const response = await fetch('http://192.168.1.227:5000/criar-usuario', {
                method: 'POST',
                headers: {
